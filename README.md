...
- **La Priorización del Atajo Inviable y la Proliferación de Antipatrones:** Bajo la premisa de "mostrar un entregable rápido a toda costa para cubrir el expediente", se impulsó en paralelo una vía alternativa desarrollada por otro compañero de la misma consultora. Esta propuesta, promovida por un perfil de desarrollo sin experiencia previa en Kubernetes ni en plataformas de contenedores corporativas, incurrió en graves antipatrones de diseño:
  - **Acoplamiento de Configuración y Recompilación Continua:** Cada cambio menor de configuración (parámetros de conexión, URLs, flags de entorno) requería recompilar el código fuente Java y reconstruir la imagen al completo, violando frontalmente el principio de inmutabilidad de contenedores y la externalización declarativa mediante *ConfigMaps* y *Secrets*.
  - **Base de Datos Efímera en Pod vs. Base de Datos Corporativa:** Se optó por levantar un entorno autocontenido con una base de datos efímera dentro del propio pod en local, soslayando la topología real de red en Red SARA, las directrices del ENS, la persistencia en el servidor SQL Server corporativo y las políticas de confinamiento Egress.
...
- **El Despropósito de los Silos Paralelos y la Competencia Artificial:** Supone una grave falta de profesionalidad y un despilfarro flagrante de recursos públicos y humanos que dos o más personas de un mismo equipo trabajen en paralelo sobre la misma tarea en absoluto aislamiento y sin comunicarse entre sí, compitiendo en una carrera artificial por ver "quién llega antes". En esta dinámica viciada, se premia la primera solución que aparenta funcionar superficialmente, aunque sea técnicamente deficiente y desaconsejable para producción, aprovechándose de que el cliente ministerial suele tener un perfil gestor y administrativo, no técnico de infraestructura.
