...
| **Despliegue Multi-Clúster (QA, PRE, PRO)** | ✅ **Nativo y Centralizado.** Kustomize `overlays/{qa,pre,prod}` orquestados por ArgoCD en clusters OCP independientes. | ⚠️ **Imperativo.** Requiere cambiar el contexto `oc login` y reejecutar scripts por cada clúster. |
